...
const TypeDropdown = () => {
  ...
  const originalPokemonList = useSelector(
    (store) => store.pokemon.originalPokemonList
  );
  ...
  const handleCategoryFilter = (e) => {
    ...
    const newList = originalPokemonList.filter(
      (pokemon) => mapp[[pokemon?.name, type1]] === 1
    );
  ...
  return (
    <div className="overflow-y-scroll h-60 no-scrollbar border-b-[1.5px] shadow-2xl bg-white text-black bg-opacity-90 xsm:absolute top-9 xsm:w-[142px] xsm:shadow-2xl">
      {pokemonTypes.map((type, idx) => (
        <p
          key={idx}
          className=" p-3 border-[1.5px] hover:bg-gray-100 xsm:hover:bg-gray-100"
